...
export interface XhsNote {
  ...
  imageUrls: string[]
  preparedImages?: XhsPreparedImage[]
  videoUrls: string[]
  videoBuffer?: Buffer
...
const LINK_PATTERNS = [
  ...
  new RegExp(`https?://xhslink\\.(?:com|cn)/${URL_BOUNDARY}`, 'gi'),
]
...
export async function resolveXhsLink(rawUrl: string, config: XhsConfigLike): Promise<string> {
  ...
  if (!isXhsShortLink(url)) return url
...
export function buildXhsMessages(note: XhsNote, config: XhsConfigLike, session?: { userId?: string, username?: string, author?: { nickname?: string } }) {
  ...
  if (config.showImages) {
    const images: XhsPreparedImage[] = note.preparedImages
      || note.imageUrls.map((source) => ({ source }))

    for (const image of images.slice(0, config.maxImages)) {
      const imageElement = typeof image.source === 'string'
        ? h.image(image.source)
        : h.image(image.source, image.mimeType || 'image/jpeg')
...
function isXhsShortLink(url: string) {
  try {
    return XHS_SHORT_LINK_HOSTS.has(new URL(url).hostname.toLowerCase())
  } catch {
    return false
  }
}
