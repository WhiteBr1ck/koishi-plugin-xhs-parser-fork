...
| `cookie` | 空 | 可选的小红书 Cookie。留空时会自动尝试通过移动端公开分享页解析。 |
| `timeout` | `15` | 网络请求超时时间，单位为秒，可设置为 3 至 60。 |
| `showError` | `false` | 解析失败时向聊天发送错误提示。 |
| `loggerinfo` | `false` | 输出链接解析、媒体下载、大小判断和失败回退等调试日志。 |

未配置 Cookie 时，插件会优先请求小红书移动端公开分享页。该方式依赖分享链接中的有效 `xsec_token`，只包含笔记 ID 且不含分享参数的链接仍可能要求登录。

## 更新日志

### 0.1.7

1. 修复未配置 Cookie 时，匿名桌面端请求被小红书重定向到登录页，导致无法读取笔记数据的问题。匿名解析现在会优先使用移动端公开分享页。
2. 修复配置 Cookie 后，小红书 `window.__INITIAL_STATE__` 中的 `new Map([])` 导致 `JSON.parse()` 失败的问题。
3. 根据是否配置 Cookie 细分解析失败提示，方便判断 Cookie 失效、笔记要求登录或链接失效等情况。
